...
class Ecg_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        ecg_data = np.resize(ecg_data,(1,1,186))
        ...
        # 心电数据预处理
        # TODO:这里的处理方式是双边零填充，最长长度187，先写死，后续有需要再补充
        # ecg_data = zero_padding(ecg_data,187)

        # 数据转格式：numpy-->tensor .unsqueeze(-2)
        ecg_data = torch.from_numpy(ecg_data).type(torch.FloatTensor)
    ...
    def add(cls, ecg_data_new, label_new, meta_new, dataset_info):
        ...
        if isinstance(ecg_data_new,torch.Tensor):
            ecg_data_new = ecg_data_new.to('cpu').detach().numpy()
